#!/usr/bin/env python3
"""Inject land polygons + data into viz/template.html -> viz/index.html."""
import json
...
tpl = (BASE / 'viz' / 'template.html').read_text()
land_path = BASE / 'viz' / 'land.json'
if not land_path.exists():
    land_path = SCRATCH / 'land.json'
land = land_path.read_text()
data = (BASE / 'viz' / 'data.json').read_text()

out = tpl.replace('/*__LAND__*/[]', land).replace('/*__DATA__*/{}', data)
(BASE / 'viz' / 'index.html').write_text(out)
print(f'viz/index.html: {len(out)/1e6:.1f} MB')
